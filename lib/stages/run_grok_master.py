"""V5.1.9 Step 4.4 Grok Master orchestrator — free-style with tool access.

Promotion of run_grok_writer.py: instead of reusing Claude Master's
data_trail, Grok now does its own research via lib.llm.research_tools.

Flow:
  1. Load article context (raw news + brief deep_question_options + format
     picks) joined from generated_news + crawl_log.
  2. Build prompt by substituting placeholders in prompts/grok_master.md.
  3. Call lib.llm.grok_master.generate_article() — SDK auto-handles
     function-calling loop (max 8 remote calls).
  4. Persist:
     - grok_* columns (title, body, word_count, model, generated_at,
       tokens_in/out, cost_usd, brief_json snapshot, step_log JSON)
     - if no other writer has filled primary fields yet, ALSO update
       title/body/word_count + recompute public_slug (Grok becomes primary).

Pipeline-safe: NEVER raises. status ∈ {success, skipped_failure,
skipped_disabled}. Pipeline orchestrator (Step 4.4) checks status and moves
on regardless.
"""
...
from lib.llm import grok_master, research_tools  # noqa: E402
...
DEFAULT_PROMPT_PATH = REPO_ROOT / "prompts" / "grok_master.md"
...
def _build_step_log(result: dict[str, Any], duration_ms: int) -> dict[str, Any]:
    ...
    render + UI code paths can read pipeline_log['step_4_3_grok_master']
...
def run_grok_master(
    ...
    *,
    ...
    """Run Grok Master for article_id. NEVER raises."""
    ...
    api_key = grok_master.load_api_key(secrets_path)
...
def _main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Step 4.4 Grok Master (V5.1.9)")
