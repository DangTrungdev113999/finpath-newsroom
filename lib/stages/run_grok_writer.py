"""Step 4.4 Grok Writer — generate parallel article via xAI Grok 4.
...
and raw news (from crawl_log) → substitutes prompts/grok_writer.md → calls
xAI API (OpenAI-compatible via lib.llm.grok_client) → persists
generated_news.grok_* columns.

Pipeline-safety: ALWAYS exits 0 even when Grok fails. Result statuses:
  - success          : generated_news.grok_* fully populated
  - skipped_failure  : status + error set; body/title NULL (Grok API failed)
  - skipped_disabled : status + error set; body/title NULL (missing api_key)

Mirrors lib/stages/run_gemini_writer.py — same article context loader,
template var builder, substitution. Only the LLM client + persistence
columns differ.

Usage:
    uv run python -m lib.stages.run_grok_writer --article-id <uuid>
"""
...
def run_grok_writer(
    ...
    *,
    ...
    """Generate Grok side for the article. NEVER raises — returns result dict.

    Result dict mirrors `lib.llm.grok_client.generate_article` shape; additionally
    `error == "article_not_found"` when the article_id has no row.
    `model` override falls back to secrets.yaml.grok.model then DEFAULT_MODEL.
    """
    if model is None:
        model = grok_client.load_model(secrets_path)
    ...
    api_key = grok_client.load_api_key(secrets_path)
...
def _main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Step 4.4 Grok Writer")
    ...
    parser.add_argument(
        ...
        help="Path to grok_writer.md prompt template",
    )
    parser.add_argument(
        "--model",
        default=None,
    ...
    # Exit 0 even on failure — pipeline MUST NOT block on Grok failure
    return 0
