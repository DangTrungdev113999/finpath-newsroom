...
Build **static knowledge base** cho ngành Bất động sản (BĐS) toàn diện, cho phép Master agent phân tích bất kỳ mã cổ phiếu BĐS Việt nào (không chỉ MVP universe). 21 file markdown tại `kb/bds/frameworks/` chứa **framework + mechanism + threshold dài hạn + case study lịch sử + pitfalls + regulatory** — kiến thức tĩnh không đổi theo quý.
...
- **21 file markdown** tại `kb/bds/frameworks/` — pure static (framework + mechanism + threshold range + pitfalls + case study lịch sử + regulatory)
...
**Tổng: 21 file** = 1 hub + 5 framework chung + 3 residential + 3 KCN + 3 retail + 2 office + 3 resort + 1 DC.
...
### Stage 2: Draft 21 file (subagent)

Cùng subagent tiếp tục viết 21 file draft v1 theo template Section 4 + 5 principles Section 5.

Output: 21 file tại `kb/bds/frameworks/*.md`.
...
- Commit atomic: `feat(kb): BĐS knowledge base — 21 files`
...
| 21 file × ~150 lines = 2700 lines KB → loader slow? | Existing kb_loader.py đã handle Bank (4 file) + CK (6 file). 21 file vẫn nhanh (regex search). |
...
- [ ] 21 file markdown tồn tại tại `kb/bds/frameworks/`
