"""Backfill Grok side for articles missing grok_status='success'.

Usage:
    uv run python scripts/backfill_grok.py [--dates YYYY-MM-DD,YYYY-MM-DD] [--dry-run]

Default dates: 2026-05-12,2026-05-13,2026-05-14 (Phase A-C ship window for
Gemini + first day of Grok parallel).
Iterates articles sequentially (xAI free tier ~60 RPM safety margin).
...
from lib.stages.run_grok_writer import run_grok_writer  # noqa: E402
...
def find_targets(db: PipelineDB, dates: list[str]) -> list[dict]:
    ...
    cur = db.conn.execute(
        f"""
        SELECT gn.article_id, gn.ticker, gn.title, gn.grok_status,
        ...
        WHERE date(cl.crawled_at) IN ({placeholders})
          AND (gn.grok_status IS NULL OR gn.grok_status != 'success')
...
def main() -> int:
    ...
    parser.add_argument(
        "--dates",
        default="2026-05-12,2026-05-13,2026-05-14",
    ...
    print(f"[backfill-grok] {len(targets)} targets across dates {dates}")
    for t in targets:
        print(f"  {t['article_id'][:8]} | {t['pub_date']} | {t['ticker']:4} | batch={t['batch_id']}")

    if args.dry_run:
        print("[backfill-grok] dry-run — no calls made")
        db.close()
        return 0
    ...
    for i, t in enumerate(targets, start=1):
        ...
        ticker = t["ticker"]
        print(
            f"[backfill-grok] {i}/{len(targets)} {ticker} {article_id[:8]} … ",
            end="",
            flush=True,
        )
        call_start = time.monotonic()
        try:
            res = run_grok_writer(article_id=article_id, db=db)
    ...
    print("\n[backfill-grok] summary:")
    print(json.dumps(summary, ensure_ascii=False, indent=2))

    summary_path = Path("/tmp/backfill-grok-summary.json")
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"\n[backfill-grok] summary written: {summary_path}")
