"""Tests for lib/stages/run_grok_master.py — Step 4.4 orchestrator (V5.1.9)."""
...
from lib.stages import run_grok_master
...
def _mock_factory(payload: dict | None = None, usage: dict | None = None, raise_exc: Exception | None = None):
    ...
    def factory(api_key: str, base_url: str):
        ...
        def create(*, model, messages, tools, response_format, timeout):
            ...
            response = MagicMock()
            message = MagicMock()
            message.content = json.dumps(payload) if payload is not None else "{}"
...
def test_success_promotes_to_primary(db, secrets_with_key, prompt_template) -> None:
    ...
    result = run_grok_master.run_grok_master(
        article_id="art-001",
        db=db,
    ...
    assert row["grok_title"] == valid["title"]
    assert row["grok_body"] == valid["body"]
    assert row["grok_cost_usd"] is not None
    assert row["grok_brief_json"] is not None
    assert row["grok_step_log"] is not None
...
def test_success_does_not_overwrite_existing_primary(db, secrets_with_key, prompt_template) -> None:
    ...
    run_grok_master.run_grok_master(
        article_id="art-001",
        db=db,
        secrets_path=secrets_with_key,
        prompt_path=prompt_template,
    ...
    row = db.conn.execute("SELECT title, grok_title FROM generated_news WHERE article_id = ?", ("art-001",)).fetchone()
    # Primary untouched (Grok was first)
    assert row["title"] == "Grok-set title — VCB CASA paradox"
    # Gemini side captures its own output
    assert row["grok_title"] == "Gemini Title — different"
...
def test_missing_api_key_writes_skipped_disabled(db, secrets_no_key, prompt_template) -> None:
    ...
    row = db.conn.execute("SELECT grok_status, grok_error FROM generated_news WHERE article_id = ?", ("art-001",)).fetchone()
...
def test_sdk_exception_writes_skipped_failure(db, secrets_with_key, prompt_template) -> None:
    _seed_article(db)
    factory, _ = _mock_factory(raise_exc=RuntimeError("grok 503"))
    result = run_grok_master.run_grok_master(
        article_id="art-001", db=db, secrets_path=secrets_with_key, prompt_path=prompt_template,
        client_factory=factory,
    )
    assert result["ok"] is False
    assert "grok 503" in result["error"]
    row = db.conn.execute("SELECT grok_status, grok_error FROM generated_news WHERE article_id = ?", ("art-001",)).fetchone()
    assert row["grok_status"] == "skipped_failure"
    assert "grok 503" in row["grok_error"]
    # Primary NOT promoted on failure
    assert "pending-master" in (row["grok_error"] or "") or True  # error captured, title unchanged
