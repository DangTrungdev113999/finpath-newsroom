...
def test_validate_step_4_5_v5_1_enforces_hard_criteria():
    """V5.1+ rows: step_4_5_headline_craft.final_title MUST pass check_hard_criteria."""
    from lib.pipeline_db import validate_pipeline_step
    import pytest
    payload = {
        "final_title": "Tin tức quý 1",  # No ticker, no hook → fails
        "final_loi": "Question",
        "picked_score": 0,
        "candidates": [],
        "hard_criteria_pass": {
    ...
    with pytest.raises(ValueError, match="hard criteria|ticker_present|hook_strong"):
...
def test_validate_step_4_5_v5_0_skips_check():
    """V5.0 row should NOT enforce step_4_5 schema (didn't exist)."""
    from lib.pipeline_db import validate_pipeline_step
    payload = {}
    validate_pipeline_step("step_4_5_headline_craft", payload, pipeline_version="V5.0")


def test_validate_step_4_5_v5_1_rejects_em_dash():
    """V1.1 PATCH: em dash in final_title → hard criteria fails (no_em_dash)."""
    from lib.pipeline_db import validate_pipeline_step
    import pytest
    payload = {
        ...
        "hard_criteria_pass": {
            ...
            "hook_strong": {"tension_present": True, "click_test_pass": True},
            "binh_dan_nguy_hiem": {"plain_language": True, "sharp_edge": True},
            "no_em_dash": False,  # CLAIMED but title actually has em dash — validate catches
        },
        "model": "claude-sonnet-4-6",
        "duration_ms": 5000,
    }
