...
- **V5.1.8 intra-batch thesis MERGE (2026-05-14)** — Trong 1 batch /tin, 2+ brief cùng (ticker, dominant_category) → KHÔNG drop. Format Director step 3.5 (`lib.intra_batch_dedup.merge_briefs_in_batch`) chọn winner (strongest signal: most options / most data_trail / highest key_metric), ENRICH winner brief: concat `deep_question_options[]` (dedup theo question text, cap 5), union `merged_key_evidence[]`. Losers marked `merge_decision='absorbed_into_<winner_row_id>'` + `master_decision='reject_dup_thesis'` (Master skip). Output: 1 article per (ticker, thesis) — content cover được cả 2 sides thay vì drop dữ liệu. V5.1.6 backward-compat: `dedup_briefs_in_batch` wrapper map merge_decision → dedup_decision (keep / drop_dup_thesis).
...
- **V5.1.7 3-model parallel writers (2026-05-14)** — Mỗi article có 3 side ngang hàng: Claude Master (Step 4), Gemini Writer (Step 4.3, `gemini-2.5-pro`), Grok Writer (Step 4.4, `grok-4.3`). Cả 3 nhận CÙNG input (brief + raw news + Claude's `data_trail`) — so sánh thuần writing quality, KHÔNG re-research. Gemini + Grok pipeline-safe (status `success`/`skipped_failure`/`skipped_disabled` — failure không halt). Web UI 3-way ModelToggle (Claude / Gemini / Grok) lưu preference qua localStorage `article.model`. SDK: `google-genai>=1.0` cho Gemini, `openai>=1.50` với `base_url='https://api.x.ai/v1'` cho Grok (OpenAI-compatible). Cost ước tính ~$0.01/article cho cả 2 parallel side (~$0.005 mỗi model). Prompt files: `prompts/gemini_writer.md` + `prompts/grok_writer.md` (initially identical copy, allowed to diverge per-model tuning).
- **V5.1.8 AI cost tracking (2026-05-14)** — Mỗi generated_news row track token in/out + USD cost per model: `claude_*`, `gemini_*`, `grok_*`, `image_cost_usd`, `total_cost_usd`. Sticker prices in `lib/llm/pricing.py` (Opus $15/$75, Sonnet $3/$15, Gemini 2.5 Pro $1.25/$5, Grok 4.3 $3/$15, Imagen 4 $0.04/img). Web RightColumn → `CostPanel` (collapsed default) hiển thị summary + per-model breakdown. 4 sections cũ (Bài gốc / Vì sao chọn / Hướng tiếp cận / Tổng biên tập) cũng collapsed default cho gọn.
- **V5.1.8 Imagen 4 thumb opt-in (2026-05-14)** — Step 4.5 Image Gen sinh hero thumbnail 1024×576 WebP per article CHỈ khi `/tin <TICKER> --image` flag được set. Default OFF. Cost $0.04/article. Module `lib/llm/imagen_client.py` + orchestrator `lib/stages/run_image_gen.py`. Prompt template `prompts/image_prompt_template.md` + sector motif `data/sector_thumb_motif.yaml` (10 sectors, drama tone, no text/logos/faces). Output to `output/thumbs/<public_slug>.webp`; URL surfaced trong frontmatter `thumb_url` + ArticleCard 16:9 hero preview trên IndexPage.
- **V5.1.8 Telegram Gemini+Grok only (2026-05-14)** — Step 7 Telegram publisher đổi format: 2-title channel post (Gemini + Grok side-by-side, KHÔNG Claude title) + thread 3 replies (Gemini body / Grok body / CTA web link). Method `TelegramPublisher.publish_article_v5(gemini_title, grok_title, gemini_body, grok_body, public_slug, total_duration_ms, total_cost_usd)`. Status `skipped_no_parallel_writers` khi cả 2 fail (no API call, no telegram_pushed_at). Single-side fallback (chỉ 1 side success) → 1 title line + 1 body reply + CTA. Legacy `publish_article_with_thread_body` còn keep cho rollback safety.
- **V5.1.9 Drop Claude Master, Gemini + Grok free-style tool calling (2026-05-14)** — Claude Master step skipped by default (flag `MASTER_ENABLED=false` trong Step 0). Gemini + Grok promoted to Master role với 8 research tools (Finpath API 5 endpoints, KB search, recent_articles, web_search via Tavily). LLM tự pick câu hỏi từ deep_question_options + tự chọn tool calls + tự log data_trail. Cost giảm từ ~$15/article xuống ~$0.15-0.35 (50× cheaper). New modules: `lib/llm/research_tools.py` (8 tool callables + dispatch), `lib/llm/gemini_master.py` (genai SDK automatic_function_calling, max 8 calls), `lib/llm/grok_master.py` (openai SDK manual tool-call loop, max 8 turns), `lib/stages/run_gemini_master.py` + `run_grok_master.py` (orchestrators with promote-to-primary logic). New DB columns: `gemini_brief_json`, `gemini_step_log`, `grok_brief_json`, `grok_step_log` (TEXT, per-side observability). Web `ModelToggle` collapse 2-way (default Gemini); `claudeAvailable` prop hides Claude button khi article không có Claude side. Legacy `lib/stages/run_{gemini,grok}_writer.py` + `prompts/{gemini,grok}_writer.md` kept for rollback. Tavily key optional — when absent, web_search tool returns `tavily_disabled` and LLM still writes using other tools + raw news.
