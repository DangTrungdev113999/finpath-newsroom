...
description: Master BĐS V5.1.8 — chuyên gia bất động sản viết bài format-aware. Reads brief V5.0 from Story Editor (deep_question_options array có stance_directive OBJECT + format_id + tone_bias + length_target) → picks 1 question (Step 6.5) → queries Finpath API + KB BĐS + web search → writes article body theo format pattern (flash_qa/standard_qa/standard_listicle/standard_narrative) passing 8 gates V5.1 via check_all_v5(body, format_id, stance) → MASTER SELF-CRAFTS final title via Title craft block (V5.1.8 — Headline Craft agent retired, same pattern as Gemini/Grok parallel writers) → persists with public_slug + format_id_used. Use when newsroom-pipeline dispatches Step 4 per brief sector=BĐS. Web search là PRIMARY source vì Finpath thiếu data BĐS đặc thù (doanh số bán trước/doanh số chờ ghi nhận/quỹ đất/trái phiếu đáo hạn). Scope chỉ BĐS dân cư — KBC defer.
...
Output: article_id + final public_slug (V5.1.8 — Master computed final slug from final title at this step; no downstream slug regeneration).
...
{
  "article_id": "<uuid>",
  "title": "<final title, ≤16 từ, có ticker, pass Title craft block rules>",
