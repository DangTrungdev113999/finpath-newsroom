# Voice Layer 5 Rules — Master Apparel

> Loaded from `Skill: finpath-newsroom-master-apparel`. Apply CROSS-CUTTING toàn bộ 4 format (flash_qa / standard_qa / standard_listicle / standard_narrative).
...
- Xấu: "Cổ phiếu có thể tăng tùy thuộc đơn hàng" (tăng/giảm đều đúng → BA PHẢI)
- Tốt: "Cổ phiếu sẽ tăng vì sổ đơn hàng quý 1 đã chốt 5 tháng" (có direction + lý do)

### Test 2 — Direction check

Có cam kết direction không? → không = fail.
- Xấu: "Vẫn còn phải chờ thêm dữ liệu mới biết"
- Tốt: "Đà tăng có thể chững lại quý 2 nếu khách Mỹ tiếp tục giãn đơn" (có direction)
...
2. **Timeframe** — `giữ trên 12 tháng`, `24-36 tháng`, `theo dõi 2 quý tới`, `lướt sóng ngắn hạn`
3. **Holder action** — `tích lũy giá oversold`, `đợi pullback`, `phòng thủ`, `chuyển hướng giá trị`

### Tốt
- "Mã phù hợp NĐT giá trị giữ trên 18 tháng, ưu tiên doanh nghiệp có tỷ trọng đơn giá xuất xưởng cao."
- "Mã có rủi ro với NĐT lướt sóng theo tin phục hồi quý vừa rồi. NĐT giá trị nên đợi xác nhận thêm 1-2 quý đơn hàng."
...
Vd:
- Story Editor brief `direction: bullish`. Master tìm thấy data confirming bullish + 1 caveat (vd USD/VND đang yếu lại). → Write bullish body, đưa caveat vào closing (Voice V3 cho phép caveat trong closing nếu confidence medium).
- Master tìm thấy data flat-out contradict bullish stance (vd sổ đơn hàng giảm xuống <3 tháng dù Story Editor cite recovery). → KHÔNG override → `master_decision: reject_data_conflict` + push back lên Story Editor (discipline 2 chiều, see SKILL.md edge cases).
